...
ctLongToWide <- function(datalong, id, time, manifestNames, TDpredNames=NULL, TIpredNames=NULL){
  ...
  TIpred_wide<-data.frame(matrix(NA,ncol=0,nrow=nrow(manifestNames_wide))) #set null dataframe to avoid errors
  if(!is.null(TIpredNames[1])){ #if there are time independent predictors
    suppressWarnings(for(i in 1:length(TIpredNames)){ #for each
