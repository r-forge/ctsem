...
#' @export
summary.ctsemMultigroupFit<-function(object,group='show chooser',timeInterval=1,...){
  ...
  output<-ctSummaryMatrices(tempctobj,ridging=FALSE, timeInterval=timeInterval)
  
  tempctobj$mxobj <- object$mxobj
  output$omxsummary<-omxSummary(tempctobj)
  return(output)
}
...
ctParameters<-ctParamsSummary(object=object,output=output)
output$ctparameters<-ctParameters
if(object$ctfitargs$transformedParams==TRUE) output$ctparammessage<-'Note: Continuous time parameter estimates above are of the full variance-covariance matrices, not cholesky decompositions as used by ctModel.'
...
omxSummary<-function(object,verbose){
  ...
  if(verbose==TRUE & object$ctfitargs$transformedParams==TRUE) output$parammessage<-'Note: Parameters and std errors displayed in OpenMx section are based on internal transformations for optimization and may not be directly interpretable.'
...
ctParamsSummary<-function(object,output=output){

  out<-matrix(NA,ncol=3,nrow=length(names(omxGetParameters(object$mxobj))))
  out[,1]<-names(omxGetParameters(object$mxobj))

  for(parami in 1:nrow(out)){ #for every free param
  for(namei in names(output)[names(output) %in% names(object$ctmodelobj)]){ #check every matrix that is in both ctmodelobj and output
    if(out[parami,1] %in% object$ctmodelobj[[namei]]) { #if the free param is in the ctmodelobj matrix
      out[parami,2]<-output[[namei]][match(out[parami,1],object$ctmodelobj[[namei]])] #put the parameter value from the output matrices into out object
    out[parami,3]<-namei
      }
  }
  }
  colnames(out)<-c('Continuous time free param', 'Value', 'Matrix')
...
ctSummaryMatrices<-function(object,ridging,timeInterval,verbose){
  ...
  if(discreteTime==FALSE){
    ...
    if(verbose==TRUE){
    MANIFESTVARdiag<-tryCatch({ diag(diag(MANIFESTVAR),n.latent)+diag(ridging,n.latent)}, error=function(e) e )
    MANIFESTVARstd<-tryCatch({ suppressWarnings(solve(sqrt(MANIFESTVAR)) %&% MANIFESTVAR)}, error=function(e) e )
...
#       outlist<-c(outlist,'TRAITVARstd')
    ...
    if(any(object$ctmodelobj$MANIFESTTRAITVAR != 0)) {
      ...
      MANIFESTTRAITVAR<-tryCatch({ mxEval(MANIFESTTRAITVAR, mxobj,compute=T)}, error=function(e) e )
