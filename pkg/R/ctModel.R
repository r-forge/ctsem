...
#' "auto" (default) freely estimates all parameters and generates random starting values.
#' 
#' @param inits an n*2 matrix where the first column contains character strings of constraint labels used in
#' the model, and the second column contains starting values for any parameters with these labels.  
#' If not set, random starting values representing relatively stable processes with small effects and 
#' covariances are generated.  
#' Better starting values may improve model fit speed and the chance of an appropriate model fit.  
#' 
#' @param initsToFixed if TRUE, uses the provided list of inits as fixed values instead of starting values
#' (convenient way to extract previous fit parameters using ctGetInits and constrain a new model)
#' @export
...
ctModel<-function(n.manifest, n.latent, Tpoints, LAMBDA, 
  ...
  #function to seperate labels and values,fixed and free,and generate start values
  fixInputMatrices <- function(x,symmetric = FALSE,diagadd = 0,addCharacters = NULL,...){
    inputm <- eval(parse(text = x)) #what matrices are we working with
    inputmFixed <- suppressWarnings(matrix(as.numeric(inputm),nrow = nrow(inputm),ncol = ncol(inputm))) #calc fixed only matrix with characters appended
    ...
    tempx <- matrix(0,nrow = nrow(inputm),ncol = ncol(inputm)) #generate fixed low correlation matrix
      ...
      for(i in 1:length(inputmCharacter)){ #for every cell in x
        if(!is.na(inputmCharacter[i])){ #if it is not NA
          if(any(inputmCharacter[i] == inits[,1])){  #and any cells match column 1 of labelinits
            message(paste0("Processing inits for ",x)) 
...
if(initsToFixed==TRUE){ #if we want to take init params as fixed instead - can be a simple way to vary models
  ...
  if(n.TDpred+n.TIpred == 0){predictorExtension <- FALSE}else{predictorExtension <- TRUE}  

  fixInputMatrices("T0VAR",rows = n.latent,cols = n.latent,symmetric = TRUE,diagadd = 0)
  fixInputMatrices("T0MEANS",rows = n.latent,cols = 1,symmetric = FALSE,diagadd = 0)
  fixInputMatrices("MANIFESTMEANS",rows = n.manifest,cols = 1,symmetric = FALSE,diagadd = 0)
  fixInputMatrices("LAMBDA",rows = n.manifest,cols = n.latent,symmetric = FALSE,diagadd = 0)
  fixInputMatrices("MANIFESTVAR",rows = n.manifest,cols = n.manifest,symmetric = TRUE,diagadd = 0)
  fixInputMatrices("DRIFT",rows = n.latent,cols = n.latent,symmetric = FALSE,diagadd = 0)
  fixInputMatrices("DIFFUSION",rows = n.latent,cols = n.latent,symmetric = TRUE,diagadd = 0)      
  fixInputMatrices("CINT",rows = 1,cols = n.latent)
  
  
  if(!is.null(TRAITVAR)){ #if needed,process and include traits in matrices
    fixInputMatrices("T0TRAITEFFECT",rows = n.latent,cols = n.latent,symmetric = FALSE,diagadd = 0,addCharacters = "FFF")
    fixInputMatrices("TRAITVAR",rows = n.latent,cols = n.latent,symmetric = TRUE,diagadd=0,addCharacters = "FFF")
  }
  
  if(!is.null(MANIFESTTRAITVAR)){ #if needed,process and include traits in matrices
    fixInputMatrices("MANIFESTTRAITVAR",rows = n.manifest,cols = n.manifest,symmetric = TRUE,diagadd=0,addCharacters = "FFF")
  }
  
  
  
  if(!is.null(TRAITTDPREDCOV)) fixInputMatrices("TRAITTDPREDCOV",symmetric = FALSE,diagadd = 0)
  
  if (n.TDpred > 0){
    fixInputMatrices("TDPREDMEANS",symmetric = FALSE,diagadd = 0)
    fixInputMatrices("TDPREDEFFECT",symmetric = FALSE,diagadd = 0)
    fixInputMatrices("T0TDPREDCOV",symmetric = FALSE,diagadd = 0)
    fixInputMatrices("TDPREDVAR",symmetric = TRUE,diagadd = 0)
    
  }
  if (n.TIpred > 0){  
    fixInputMatrices("TIPREDMEANS",symmetric = FALSE,diagadd = 0)
    fixInputMatrices("TIPREDEFFECT",symmetric = FALSE,diagadd = 0)
    fixInputMatrices("TIPREDVAR",symmetric = TRUE,diagadd = 0)
    fixInputMatrices("T0TIPREDEFFECT",symmetric = FALSE,diagadd = 0)
  }
  
  if(n.TDpred > 0 & n.TIpred > 0) fixInputMatrices("TDTIPREDCOV",symmetric = FALSE,diagadd = 0)
